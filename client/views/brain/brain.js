  ...
  if (TWEEN.getAll().length == 0) {
    ...
    switch (state) {
      ...
      case 10:
        ...
        new TWEEN.Tween(planeMaterial).to({
          opacity: 0
        }, 1000).easing(TWEEN.Easing.Linear.None).start().onComplete(function() {
...
var container, stats;
var camera, scene, renderer, particles, geometry, n, m, state = 0,
  velocities, brain, brain_json, brainMaterial, brainBox, brainBoxMaterial, sparklines;
var mouseX = 0,
  mouseY = 0;
var time, last_time;
var sprite;
...
function grid() {
  ...
  new TWEEN.Tween(particleSystem.rotation).to({
    y: next_angle
  }, 2000).easing(TWEEN.Easing.Quadratic.InOut).start();
...
function wireFrameBrain() {
  ...
  brainMaterial = new THREE.MeshBasicMaterial({
    color: new THREE.Color(0x29abe2),
    wireframe: true,
    opacity: 0,
    transparent: true,
    blending: THREE.AdditiveBlending,
  ...
  new TWEEN.Tween(brainMaterial).to({
    opacity: 0.4
...
function brainBox() {
  ...
  brainBoxMaterial = new THREE.MeshLambertMaterial({
    color: new THREE.Color(0x29abe2),
    transparent: true,
    opacity: 0
  });
  ...
  scene.add(brainBox);
  new TWEEN.Tween(brainBoxMaterial).to({
    opacity: 1
  }, 2000).easing(TWEEN.Easing.Quadratic.InOut).start();
...
function closeBrainBox() {
  brainBox.material = new THREE.MeshLambertMaterial({
...
function circle() {
  ...
  new TWEEN.Tween(brain.scale).to({
    x: 0,
    y: 0,
    z: 0
  }, 2000).easing(TWEEN.Easing.Quadratic.InOut).start();
...
function men() {
  ...
  for (var i = 0; i < points_head2.length; i++) {
    points_head2[i].x += 300;
  }
  for (var i = 0; i < points_body2.length; i++) {
    points_body2[i].x += 300;
  }

  for (var i = 0; i < points_head3.length; i++) {
    points_head3[i].x -= 300;
  }
  for (var i = 0; i < points_body3.length; i++) {
    points_body3[i].x -= 300;
  }
...
function loadSparklines() {
  sparkline_material1 = new THREE.MeshBasicMaterial({
    color: new THREE.Color(0x29abe2),
    transparent: true,
    opacity: 0,
    blending: THREE.AdditiveBlending
  });
  sparkline_material2 = new THREE.MeshBasicMaterial({
    color: new THREE.Color(0x90d4d5),
    transparent: true,
    opacity: 0,
    blending: THREE.AdditiveBlending
  });
...
function showSparklines() {
  new TWEEN.Tween(sparkline_material1).to({
    opacity: 0.2
  }, 2000).easing(TWEEN.Easing.Linear.None).start();
  new TWEEN.Tween(sparkline_material2).to({
    opacity: 0.2
  }, 2000).easing(TWEEN.Easing.Linear.None).start();
...
function network() {
  ...
  network_line_material = new THREE.LineBasicMaterial({
  ...
  new TWEEN.Tween(network_line_material).to({
    opacity: 1
  }, 2000).easing(TWEEN.Easing.Linear.None).delay(500).start();
  new TWEEN.Tween(sparkline_material1).to({
    opacity: 0
  }, 2000).easing(TWEEN.Easing.Linear.None).start();
  new TWEEN.Tween(sparkline_material2).to({
...
function animateNetwork() {
  for (var i = 0; i < 250; i++) {
    geometry.vertices[i].x += geometry.velocities[i].x;
    geometry.vertices[i].y += geometry.velocities[i].y;
    if (geometry.vertices[i].x > 750) {
      geometry.vertices[i].x = -750
    }
    if (geometry.vertices[i].x < -750) {
      geometry.vertices[i].x = 750
    }
    if (geometry.vertices[i].y > 750) {
      geometry.vertices[i].y = -750
    }
    if (geometry.vertices[i].y < -750) {
      geometry.vertices[i].y = 750
    }
